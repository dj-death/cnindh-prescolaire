...
var Service = {
    list: function (params, callback, sid, req) {
        session.verify(req).then(function (session) {
            ...
            return models.Activite.scope('browse').findAndCountAll(
                helpers.sequelizify(params, models.Activite));
    ...
    insert: function (params, callback, sid, req) {
        ...
        session.verify(req).then(function (session) {
            ...
            if (params.dao_id) {
                params.daoId = params.dao_id
            } else {
                callback(new Error('Aucune DAO rattachée !'));
                return;
            }

            return models.Activite.findOne({
                where: {
                    dao_id: params.daoId,
            ...
            return models.Activite.create(params, { include: [{ model: models.Dao, as: 'dao' }] });
        }).then(function (row) {
    ...
    update: function (params, callback, sid, req) {
        ...
        session.verify(req).then(function (session) {
            ...
            if (!row) {
                throw errors.types.invalidParams({
                    path: 'id', message: 'Activité with the specified id cannot be found',
                });
            }
    ...
    filters: function (params, callback, sid, req) {
        session.verify(req).then(function () {
            return helpers.fetchFilters(params, models.Activite);

...
var Config = exports.Config = {
  prefix: "measure-tool",
  tooltipText1: 'Faire glisser pour modifier, cliquer pour supprimer',
  tooltipText2: 'Faire glisser pour modifier'
};
...
var UniteionUtility = function () {
  function UniteionUtility(div, uniteion, options) {
    _classCallCheck(this, UniteionUtility);
    ...
    this._options = Object.assign({}, this._defaultOptions, options || {});
    this._container = div;
    this._uniteion = uniteion;
  }

  _createClass(UniteionUtility, [{
    key: "latLngToSvgPoint",
    value: function latLngToSvgPoint(coords) {
      var rate = this._options.offsetRate / 2;
      var latLng = new google.maps.LatLng(coords[1], coords[0]);
      var svgPoint = this._uniteion.fromLatLngToDivPixel(latLng);
    ...
    value: function svgPointToLatLng(point) {
      ...
      var coords = this._uniteion.fromDivPixelToLatLng(svgPoint);
    ...
    value: function svgPointToContainerPoint(point) {
      ...
      return this._uniteion.fromLatLngToContainerPixel(new google.maps.LatLng(svgPoint[1], svgPoint[0]));
    }
  }, {
    key: "latLngToContainerPoint",
    value: function latLngToContainerPoint(coords) {
      return this._uniteion.fromLatLngToContainerPixel(new google.maps.LatLng(coords[1], coords[0]));
    }
  }]);

  return UniteionUtility;
}();

exports.default = UniteionUtility;
;
module.exports = exports["default"];
...
var _uniteionUtility = __webpack_require__(25);

var _uniteionUtility2 = _interopRequireDefault(_uniteionUtility);
  ...
  _createClass(MeasureTool, [{
    ...
    value: function _init() {
      ...
      if (this._options.contextMenu) {
        this._contextMenu = new _contextMenu2.default(this._containerDiv, { width: 170 });
        this._startElementNode = this._contextMenu.addItem("Mesurer une Distance", true, this.start, this);
        this._endElementNode = this._contextMenu.addItem("Effacer les Mesures", false, this.end, this);
    ...
    value: function _bindToggleContextMenu() {
      ...
      this._map.addListener('rightclick', function (mouseEvent) {
        ...
        _this._contextMenu.show(_this._uniteion.fromLatLngToContainerPixel(mouseEvent.latLng));
      });
    ...
    value: function _onAddOverlay() {
      ...
      this._uniteion = this._overlay.getUniteion();
      this._uniteionUtility = new _uniteionUtility2.default(this._containerDiv, this._uniteion);
      // Add svg to Pane
    ...
    value: function _updateCircles() {
      ...
      var circles = this._nodeCircles.selectAll("circle").data(this._geometry ? this._geometry.nodes : []).attr('class', function (d, i) {
        ...
        return _this3._uniteionUtility.latLngToSvgPoint(d)[0];
      }).attr('cy', function (d) {
        return _this3._uniteionUtility.latLngToSvgPoint(d)[1];
    ...
    value: function _updateLine() {
      ...
      var linesBase = this._linesBase.selectAll("line").data(this._geometry ? this._geometry.lines : []).attr("class", "base-line").attr('x1', function (d) {
        return _this4._uniteionUtility.latLngToSvgPoint(d[0])[0];
      }).attr('y1', function (d) {
        return _this4._uniteionUtility.latLngToSvgPoint(d[0])[1];
      }).attr('x2', function (d) {
        return _this4._uniteionUtility.latLngToSvgPoint(d[1])[0];
      }).attr('y2', function (d) {
        return _this4._uniteionUtility.latLngToSvgPoint(d[1])[1];
      }).each(function (d) {
      ...
      var linesAux = this._linesAux.selectAll("line").data(this._geometry ? this._geometry.lines : []).attr("class", "aux-line").attr('x1', function (d) {
        ...
        var point = _helper2.default.findTouchPoint([_this4._uniteionUtility.latLngToSvgPoint(d[0]), _this4._uniteionUtility.latLngToSvgPoint(d[1])], [_d3Selection.event.offsetX, _d3Selection.event.offsetY]);
    ...
    value: function _updateSegmentText() {
      ...
      var text = this._segmentText.selectAll("text").data(this._geometry ? this._geometry.lines : []).attr('class', 'segment-measure-text').attr('text-anchor', 'middle').attr('dominant-baseline', 'text-before-edge').attr('transform', function (d) {
        var p1 = _this5._uniteionUtility.latLngToSvgPoint(d[0]);
        var p2 = _this5._uniteionUtility.latLngToSvgPoint(d[1]);
    ...
    value: function _updateNodeText() {
      ...
      var text = this._nodeText.selectAll("text").data(this._geometry ? this._geometry.nodes : []).attr('class', function (d, i) {
        ...
        return _this6._uniteionUtility.latLngToSvgPoint(d)[0];
    ...
    value: function _onOverCircle(d, i, target) {
      ...
      if (this._options.tooltip) {
        this._tooltip.show(this._uniteionUtility.latLngToContainerPoint(d), i === 0 ? _config.Config.tooltipText2 : _config.Config.tooltipText1);
      }
    }
  }, {
    key: '_onOutCircle',
    ...
    value: function _onDragCircle() {
      ...
      var circleDrag = (0, _d3Drag.drag)().on('drag', function (d, i) {
        ...
        self._updateArea(i, self._uniteionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
      });
      ...
      circleDrag.on('end', function (d, i) {
        ...
        if (!isDragged) {
          ...
          self._geometry.updateNode(i, self._uniteionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
    ...
    value: function _onDragLine() {
      ...
      var lineDrag = (0, _d3Drag.drag)().on('drag', function (d, i) {
        ...
        if (!isDragged) {
          isDragged = true;
          _this7._geometry.insertNode(i + 1, _this7._uniteionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
          _this7._updateLine();
        ...
        _this7._updateArea(i + 1, _this7._uniteionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
      });
      ...
      lineDrag.on('end', function (d, i) {
        ...
        if (isDragged) {
          _this7._geometry.updateNode(i + 1, _this7._uniteionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]));
    ...
    value: function _updateSegmentTextPosition(index) {
      ...
      if (index < this._geometry.lines.length) {
        this._segmentText.select('text:nth-child(' + (index + 1) + ')').attr('transform', function (d) {
          ...
          var p2 = _this8._uniteionUtility.latLngToSvgPoint(d[1]);
          return _helper2.default.transformText(p1, p2);
        }).text(function (d) {
          return _this8._helper.formatLength(_this8._helper.computeLengthBetween(_this8._uniteionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y]), d[1]));
        });
      }
      if (index > 0) {
        this._segmentText.select('text:nth-child(' + index + ')').attr('transform', function (d) {
          var p1 = _this8._uniteionUtility.latLngToSvgPoint(d[0]);
          var p2 = [_d3Selection.event.x, _d3Selection.event.y];
          return _helper2.default.transformText(p1, p2);
        }).text(function (d) {
          return _this8._helper.formatLength(_this8._helper.computeLengthBetween(d[0], _this8._uniteionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y])));
        });
      }
    }
  }, {
    ...
    value: function _updateNodeTextPosition(index) {
      ...
      this._nodeText.select('text:nth-child(' + (index + 1) + ')').attr('x', _d3Selection.event.x).attr('y', function () {
        var offset = void 0;
        if (index > 0 && _this9._uniteionUtility.latLngToSvgPoint(_this9._geometry.nodes[index - 1])[1] < _d3Selection.event.y) {
          offset = 23;
        } else {
          offset = -7;
        }
      ...
      this._nodeText.select('text:nth-child(' + (index + 2) + ')').attr('y', function (d) {
        var offset = void 0;
        if (index + 1 > 0 && _d3Selection.event.y < _this9._uniteionUtility.latLngToSvgPoint(d)[1]) {
          offset = 23;
        } else {
          offset = -7;
        }
        return _this9._uniteionUtility.latLngToSvgPoint(d)[1] + offset;
      });
      var followingNodes = this._nodeText.selectAll('text').filter(function (d, i) {
        return i >= index;
      });
      followingNodes.text(function (d, i) {
        var len = _this9._helper.computePathLength([].concat(_toConsumableArray(_this9._geometry.nodes.slice(0, index)), [_this9._uniteionUtility.svgPointToLatLng([_d3Selection.event.x, _d3Selection.event.y])], _toConsumableArray(_this9._geometry.nodes.slice(index + 1, index + 1 + i))));
    ...
    value: function _updateHoverCirclePosition(x, y) {
      ...
      if (this._dragging) return;
      if (this._options.tooltip) {
        this._tooltip.show(this._uniteionUtility.svgPointToContainerPoint([x, y]), _config.Config.tooltipText2);
      }
    }
  }, {
    ...
    value: function _transformNodeTextY(d, i) {
      ...
      return this._uniteionUtility.latLngToSvgPoint(d)[1] + offset;
    }
  }, {
    key: '_updateArea',
    value: function _updateArea(i, pointToCompare) {
      ...
      this._area = area;
      if (area > 0) {
        this._nodeText.select(':last-child').text('Distance totale: ' + this.lengthText + '; Supérficie totale: ' + this.areaText + '.');
      }
    }
  }, {
    ...
    value: function _showTooltipOnEvent(text) {
      if (this._options.tooltip) {
        this._tooltip.show(this._uniteionUtility.svgPointToContainerPoint([_d3Selection.event.x, _d3Selection.event.y]), text);
      }
    }
  }, {
    key: '_hideTooltip',
